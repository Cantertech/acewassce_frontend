...
async def router_node(state: GradingState):
    ...
    for sub in state['submissions']:
        try:
            ...
            # Priority: 1. question_number column, 2. feedback column (old tags)
            raw_tags = sub.get("question_number") or sub.get("feedback") 
            
            q_nums = []
            
            if raw_tags and str(raw_tags).strip().lower() != "unknown":
                import re
                # Split by comma or space and clean up
                raw_parts = re.split(r'[,|\s]', str(raw_tags))
                ...
                print(f"DEBUG [Router]: Using Tags: {q_nums}")
            else:
                print(f"DEBUG [Router]: Image skipped (No question number provided)")
                q_nums = []
...
async def batch_grade_node(state: GradingState):
    ...
    for urls, q_nums in image_groups.items():
        ...
        eval_prompt = (
            ...
            "4. BLANK SECTIONS / MISSING STEPS: You must NEVER assume, infer, guess, or hallucinate any calculations, formulas, or answers that are not physically written on the student's sheet. If a subquestion, subpart, or step (e.g. part a, part b(ii), area calculation, etc.) is blank or left uncalculated on the student's paper, you MUST award exactly 0 marks for that part. Write '-M1', '-A1', or '-B1' for every missing component and explain that 'this section is blank / no workings shown'. Do NOT grant marks for 'reasoned from context' or 'implicitly shown'. If it is not handwritten on the sheet, it does not exist.\n\n"
            "INSTRUCTIONS:\n"
